  ...
  _resolve: function (addressObject) {
    ...
    return new Promise(function(resolve, reject){
      if (addressObject.prefix === 'acct') {
        _this.gatewayd.models.externalAccounts.find({where: {address: addressObject.address}})
          .success(function(externalAccount) {
            if (!externalAccount) {
              reject(new Error('GatewayUserNotFound'));
            } else {
              resolve({
                address: 'ripple:' + _this.gatewayd.config.get('COLD_WALLET')+'?dt=' + externalAccount.dataValues.id
              });
            }
          }).error(reject);
      }
